...
Robbery_model <- glm.nb(Robbery_rate ~ Income_scaled + Unemployme_scaled, data = ct_data)
summary(Robbery_model)

Auto_Theft_model <- glm.nb(Auto_Theft_rate ~ Income_scaled, data = ct_data)
summary(Auto_Theft_model)

Break_and_Enter_model <- glm.nb(Break_and_rate ~ Income_scaled + I(Unemployme_scaled^2), data = ct_data)
summary(Break_and_Enter_model)

Theft_Over_5000_model <- glm.nb(Theft_Over_rate ~ Income_scaled + I(Unemployme_scaled^2), data = ct_data)
...
extract_model_coefficients <- function(model, crime_name) {
  ...
  tibble(
    ...
    z.value = coefs[, "z value"],  # Adjusted from t.value to z.value
    Pr = coefs[, "Pr(>|z|)"],      # Adjusted from Pr(>|t|) to Pr(>|z|)
    Crime = crime_name
  )
}

# Assuming the model variables Robbery_model, Auto_Theft_model, Break_and_Enter_model, Theft_Over_5000_model
# are defined and are the outputs from glm.nb()
models <- list(Robbery = Robbery_model, Auto_Theft = Auto_Theft_model, 
               Break_and_Enter = Break_and_Enter_model, Theft_Over_5000 = Theft_Over_5000_model)
...
robbery_lag_model <- lagsarlm(Robbery_rate ~ Income_scaled + Unemployme_scaled, data=ct_data, listw=robbery_listw)
...
# Example model fit using the spatial weights
auto_lag_model <- lagsarlm(Auto_Theft_rate ~ Income_scaled + Unemployme_scaled, data=ct_data, listw=auto_listw)
...
# Example model fit using the spatial weights
break_lag_model <- lagsarlm(Break_and_rate ~ Income_scaled + I(Unemployme_scaled^2), data = ct_data, listw=break_listw)
...
# Example model fit using the spatial weights
theft_over_lag_model <- lagsarlm(Theft_Over_rate ~ Income_scaled + I(Unemployme_scaled^2), data = ct_data, listw=theft_over_listw)
...
```{r}
# List of models with corresponding names
lag_models <- list(Robbery = robbery_lag_model, Auto_Theft = auto_lag_model, 
               Break_and_Enter = break_lag_model, Theft_Over_5000 = theft_over_lag_model)

lag_extract_model_coefficients <- function(lag_model, crime_name) {
  # Extracting coefficients and their names
  lag_coefs <- summary(lag_model)$coefficients
  std_errors = summary(lag_model)$rest.se
  
  # Building the data frame
  tibble(
    Predictor = names(lag_coefs),
    Coefficient = lag_coefs,
    Std.Error = std_errors,
    # Optionally calculate z-value if needed:
    # z.value = lag_coefs / std_errors,
    Crime = crime_name
  )
}

# Apply the modified function to each model and bind rows together
lag_coef_data <- bind_rows(lapply(names(lag_models), function(name) {
  lag_extract_model_coefficients(lag_models[[name]], name)
}), .id = "ModelID")

# Print the resulting data frame to see the outcome
print(lag_coef_data)


ggplot(lag_coef_data, aes(x = Predictor, y = Coefficient, fill = Crime)) +
  geom_bar(stat = "identity", position = position_dodge()) +  # Use position_dodge to separate bars per group
  labs(title = "Comparative Effects of Predictors Across Crime Types",
       y = "Coefficient Value", 
       x = "Predictor Variables") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))  # Adjust text alignment for readability
...
ggplot(data = ct_data) +
  ...
  labs(fill = "Predicted Robbery Rate (Lag Model)") +
  theme_minimal()
...
ggplot(data = ct_data) +
  ...
  scale_fill_distiller(palette = "YlOrRd", direction = 1, labels = scales::comma) +
  labs(fill = "Predicted Auto Theft Rate (Lag Model)") +
  theme_minimal()
...
ggplot(data = ct_data) +
  ...
  scale_fill_distiller(palette = "YlOrRd", direction = 1, labels = scales::comma) +
  labs(fill = "Predicted Break and Enter Rate (Lag Model)") +
  theme_minimal()
...
ggplot(data = ct_data) +
  ...
  scale_fill_distiller(palette = "YlOrRd", direction = 1, labels = scales::comma) +
  labs(fill = "Predicted Theft Over $5000 Rate (Lag Model)") +
  theme_minimal()
```
